...
The example uses the Twitter US Airline Sentiment dataset, which contains 14,640 airline-related tweets [@TwitterAirlineSentiment]. The dataset includes the tweet text, the airline, a sentiment label, and, for negative tweets, a reason for the negative sentiment.

```{r}
tweets |>
  count(airline_sentiment) |>
  mutate(share = scales::percent(n / sum(n))) |>
  knitr::kable(caption = "Sentiment distribution in the airline tweet dataset")
```
...
This step follows an important principle in reproducible data analysis: the workflow should be written in code so that the analysis can be repeated or adjusted later [@Huber2025RNotes].
...
The dataset contains a variable that explains the reason behind many negative tweets. The following output shows the most frequent reasons for negative sentiment.

```{r}
negative_reasons |>
  slice_max(n, n = 8) |>
  knitr::kable(caption = "Most frequent reasons for negative sentiment")
```
...
A central step in text mining is tokenization. Tokenization means splitting text into smaller units. In this example, each tweet is split into individual words. Stop words such as "the", "and", or "is" are removed because they usually do not provide much meaning for sentiment analysis.
...
The tidy approach is useful because the result is a table in which each row is one word from one tweet. This format can be filtered, counted, joined with dictionaries, and visualised using familiar `dplyr` and `ggplot2` functions [@Silge2017Text].
...
This method is transparent and easy to explain. However, it has limitations. It may miss sarcasm, context, negation, and domain-specific meanings. For example, the word "delay" may clearly be negative in an airline context, while other words may depend strongly on context.
...
top_sentiment_words |>
  ...
  group_by(sentiment) |>
  slice_head(n = 5) |>
  knitr::kable(caption = "Frequently matched sentiment words")
```


# Additional Visual Analysis Used in the Presentation

The presentation includes several visual outputs that support the interpretation of the analysis. To keep the handout consistent with the presentation, the following figures show how these outputs were produced in R.

## Main Negative Reasons

The following plot visualizes the most frequent reasons behind negative airline tweets. This is important for decision-making because each reason represents a possible operational problem.
...
After tokenizing the tweets and matching the words with the Bing sentiment lexicon, the following plot shows the most frequent positive and negative sentiment words. This helps make the lexicon-based sentiment analysis more interpretable.
...
The following plot provides an additional lexicon-based comparison across airlines. For each airline, the number of positive and negative sentiment words is counted. The score is calculated as positive words minus negative words, standardized per 100 sentiment words.
...
These figures are used as supporting evidence. The final decision-making step still requires managerial interpretation, because lexicon-based sentiment analysis can miss sarcasm, context, or ambiguous meanings.
...
decision_actions |>
  ...
  head(8) |>
  knitr::kable(caption = "Converting negative sentiment reasons into managerial actions")
```
